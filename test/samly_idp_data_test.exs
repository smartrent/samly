defmodule SamlyIdpDataTest do
  ...
  test "valid-idp-config-1" do
    %IdpData{} = idp_data = IdpData.load_provider(@idp_config1)
    assert idp_data.valid?
  end

  # verify defaults
  test "valid-idp-config-2" do
    %IdpData{} = idp_data = IdpData.load_provider(@idp_config1)
  ...
  test "valid-idp-config-3" do
    ...
    %IdpData{} = idp_data = IdpData.load_provider(idp_config)
    refute idp_data.use_redirect_for_req
    assert idp_data.sign_requests
    assert idp_data.sign_metadata
    assert idp_data.signed_assertion_in_resp
  ...
  test "sp entity_id test-1" do
    %IdpData{} = idp_data = IdpData.load_provider(@idp_config2)
    assert idp_data.valid?
  ...
  @tag :skip
  test "invalid-idp-config-1" do
    idp_config = %{@idp_config1 | id: ""}
    %IdpData{} = idp_data = IdpData.load_provider(idp_config)
    refute idp_data.valid?
  end

  test "invalid-idp-config-2" do
    idp_config = %{@idp_config1 | sp_id: "unknown-sp"}
    %IdpData{} = idp_data = IdpData.load_provider(idp_config)
    refute idp_data.valid?
  end

  test "valid-idp-config-signing-turned-off" do
    idp_config =
      Map.merge(@idp_config1, %{
        sp_id: "sp5",
        use_redirect_for_req: true,
  ...
  test "invalid-idp-config-signing-on-cert-missing" do
    idp_config =
      Map.merge(@idp_config1, %{
        sp_id: "sp3",
        use_redirect_for_req: true,
  ...
  test "invalid-idp-config-signing-on-key-missing" do
    idp_config =
      Map.merge(@idp_config1, %{
        sp_id: "sp4",
        use_redirect_for_req: true,
  ...
  test "nameid-format-in-metadata-but-not-config-should-use-metadata" do
    %IdpData{} = idp_data = IdpData.load_provider(@idp_config1)
    assert idp_data.nameid_format == 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient'
  end

  test "nameid-format-in-config-but-not-metadata-should-use-config" do
    idp_config =
      Map.merge(@idp_config1, %{
  ...
  test "nameid-format-in-metadata-and-config-should-use-config" do
    idp_config =
      Map.merge(@idp_config1, %{
  ...
  test "nameid-format-in-neither-metadata-nor-config-should-be-unknown" do
    idp_config =
      Map.merge(@idp_config1, %{
        metadata_file: "test/data/shibboleth_idp_metadata.xml"
      })
